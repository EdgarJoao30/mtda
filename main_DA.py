...
# custom modules
from py_module.data_module_DA import Koumbia_DataModule
from py_module.task_module_DA import ClassifyTimeSeries
from py_module.callbacks_DA import get_callbacks
...
class NN:
    ...
    def run(self):
        ...
        # set up data loader
        dm = Koumbia_DataModule(self.batch_size, self.in_domain, self.out_domain)

        # set up model
        model = eval(f"{self.model}(8)")
        criterion = eval(f"{self.loss}()")
        criterion_pseudo = eval(f"{self.loss}(reduction = 'none')")
        ...
        best_f1 = self.callbacks[0].state_dict()["best_model_score"]
        self.wandb_logger.experiment.summary["best_MulticlassF1Score"] = best_f1

        # load best model & test
        trainer.test(TS_module, datamodule=dm, ckpt_path='last') #
